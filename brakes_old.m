% Patrick Eleazar and George Liu
% 8/31/25
...
arrl = (10*max_speed) + 1;  % global array length defined by max speed
...
wt = zeros(1,arrl);                     % weight transfer forwards under max decel. at different speeds
fx_f_braking = zeros(1,arrl);           % front axle longitudinal force under max decel. at different speeds
fx_r_braking = zeros(1,arrl);           % rear axle longitudinal force under max decel. at different speeds
...
f_pedal_force_init = zeros(1,arrl);     % front pedal force after initial traction-limited calculation
r_pedal_force_init = zeros(1,arrl);     % rear pedal force after initial traction-limited calculation
decels_actual = zeros(1,arrl);          % "actual" max. decel. at different speeds given both traction and pedal force limitations
decels_init = zeros(1,arrl);            % stores initial traction-limited decelerations
pedal_force_delta = zeros(1,arrl);      % stores delta between front and rear pedal forces at max deceleration
...
while abs(decels(arrl) - decels_actual(arrl)) > 0.0001   % will converge quickly
...
%%
% figure
% plot((0:arrl-1)/10, lowest_pedal_force(1:arrl))
% legend("Pedal Force")
% xlabel("Speed (mph)")
...
figure
plot(decels(1:arrl), f_pedal_force(1:arrl))
legend("Pedal Force")
...
figure
plot((0:arrl-1)/10, f_pedal_force_init(1:arrl), (0:arrl-1)/10, r_pedal_force_init(1:arrl))
